...
struct Counters {
  ...
  friend std::ostream &operator<<(std::ostream &os, const Counters &o) {
    return os << "Comp:" << o.comparisons << " Ops:" << o.structural_ops
              << " Inserts: " << o.inserts << std::endl;
  }

  void saveCounters(const std::string &filename = "counters.json",
                    bool asDict = true) const {
    json j;
    ...
    if (asDict) {
      ...
      j = {comparisons, structural_ops, inserts,
           deletes,     lookups,        resize_events};
    }
